...
contract ThunderLoan is Initializable, OwnableUpgradeable, UUPSUpgradeable, OracleUpgradeable {
    ...
    /*//////////////////////////////////////////////////////////////
                            STATE VARIABLES
    //////////////////////////////////////////////////////////////*/
    // e I think it maps the the underlying token to it's assetToken
    // e USDC -> USDCAssetToken
    mapping(IERC20 => AssetToken) public s_tokenToAssetToken;

    // The fee in WEI, it should have 18 decimals. Each flash loan takes a flat fee of the token price.
    // q why is this a storage variable?
    ...
    /*//////////////////////////////////////////////////////////////
                           EXTERNAL FUNCTIONS
    //////////////////////////////////////////////////////////////*/
    // @audit-info change name to poollFactoryAddress
    // q what happens if we deployed the contract, and somone else initializes it?
    // a that would suck
    ...
    function initialize(address tswapAddress) external initializer {
        ...
        __Oracle_init(tswapAddress); // eusing tswap as some kind of oracle perhaps?
        s_feePrecision = 1e18; //@audit-info written in aderyn magic numbers
    ...
    function deposit(IERC20 token, uint256 amount) external revertIfZero(amount) revertIfNotAllowedToken(token) {
        ...
        // we shouldn't updating the exchange rate here!
        uint256 calculatedFee = getCalculatedFee(token, amount);
        // q why are we updating the exchange rate?
        assetToken.updateExchangeRate(calculatedFee);
        // e when a liqudity provider deposits, the $ sits in the assetToken contract
        ...
        revertIfNotAllowedToken(token)
    {
        ...
        // 1e18 * 1e18 / 1e18 = 2e18
        // 1e18 * 2e18 / 1e18 = 2e18
        // 1e6 (USDC 6 decimal) * 2e18 / 1e18 = 2e6 (USDC 6 decimal)
        ...
        revertIfNotAllowedToken(token)
    {
        ...
        // @audit-info messed up slither-info
        // slither-disable-next-line unused-return reentrancy-vulnerabilities-2
        // @follow-up reentrancy
        // @follow-up do we need to return the value of function call()
    ...
    function setAllowedToken(IERC20 token, bool allowed) external onlyOwner returns (AssetToken) {
        if (allowed) {
            ...
            AssetToken assetToken = s_tokenToAssetToken[token];
            delete s_tokenToAssetToken[token]; // does deleting mapping work right??
            emit AllowedTokenSet(token, assetToken, allowed);
            return assetToken;
        }
    }
    ...
    // qans is this calculating the fees of flash loans?
    // ans yes!
    ...
    function getCalculatedFee(IERC20 token, uint256 amount) public view returns (uint256 fee) {
        ...
        // 1 USDC + 0.003 WETH
        // 1 USDC + 0.003 USDC???
        // @audit  IMPACT prices are wrong -> medium/high LIKELIHOOD -> high
        // @audit high - if the fee is going to be in the token, then the values should reflect that
